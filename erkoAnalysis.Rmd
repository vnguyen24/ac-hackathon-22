...
```{r attendance at tip each game based on promo both seasons, echo=FALSE}
shapes <- c(3, 16, 17, 15)
names(shapes) <- c("None", "For All", "First 10K", "First 5K")

ggplot(data = promo_timeline, 
       aes(x = month_day, y = Attendance.at.tip, shape = type_promo, color = Season)) + 
  geom_point() + 
  geom_path(aes(x = month_day, y = Attendance.at.tip, color = Season), 
            inherit.aes = FALSE) +
  scale_shape_manual(values = shapes) +
